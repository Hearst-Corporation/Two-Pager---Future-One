...
export const TOOL_DEFS = [
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        rationale: { type: 'string', minLength: 20, description: 'REQUIRED, min 20 chars. Why this change is being made. Will appear in hearst_audit_log.impact_description.' },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        caveat:                 { type: 'string' },
        rationale:              { type: 'string', minLength: 20, description: 'REQUIRED, min 20 chars. Why this source is being added (relevance to the metric and project).' },
      },
      required: ['metric_id', 'metric_name', 'source_name', 'source_type', 'rationale'],
    },
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        rationale:   { type: 'string', minLength: 20, description: 'REQUIRED, min 20 chars. Why this source is being attached to this field.' },
      },
      required: ['scenario_id', 'field_name', 'source_id', 'rationale'],
    },
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        rationale:         { type: 'string', minLength: 20, description: 'REQUIRED, min 20 chars. Why this scenario is being created.' },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        notes:                 { type: 'string' },
        rationale:             { type: 'string', minLength: 20, description: 'REQUIRED, min 20 chars. Why this prospect is being added (origin, strategic fit).' },
      },
      required: ['prospect_name', 'prospect_type', 'rationale'],
    },
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        rationale: { type: 'string', minLength: 20, description: 'REQUIRED, min 20 chars. Why this status/notes change is being made.' },
      },
      required: ['item_id', 'rationale'],
    },
  },
  {
  ...
  {
    name: 'navigate',
    description: 'Navigate the user to a Hearst cockpit page. Use when the user asks to "go to", "open", "show me" a section. No persistence: emits a client_action that the frontend dispatches to router.push.',
    input_schema: {
      type: 'object',
      properties: {
        path:   { type: 'string', description: 'A whitelisted path like /admin/hearst/simulator. Only paths under /admin/hearst/* are accepted.' },
        reason: { type: 'string', description: '1-line user-facing reason for the navigation.' },
      },
      required: ['path'],
    },
  },
  {
    ...
    description: 'Set a parameter of the investment simulator. Only effective when the user is currently on /admin/hearst/simulator. No persistence: emits a client_action that the frontend dispatches to the reducer.',
    input_schema: {
      type: 'object',
      properties: {
        field: { type: 'string', description: 'One of: total_mw, capital_usd, target_irr_pct, primary_archetype_id, business_model_id, client_type_id, mode, geography, hardware_mix.classic_pct, hardware_mix.liquid_pct, hardware_mix.ai_pct, hardware_mix.gpu_sku_id, hardware_mix.utilization_pct, hardware_mix.gpu_hour_price' },
        value: { description: 'New value (type depends on field — number for MW/pcts, string for ids)' },
...
// Client-side tools — pas de mutation DB ni de fetch externe.
// runTool retourne juste un payload { ok, client_action } que le frontend
// (ChatContainer) dispatchera via router.push ou window.dispatchEvent.
const NAV_PATH_WHITELIST = new Set([
  '/admin/hearst',
...
// Minimum length for a mutating-tool rationale. Anything shorter is almost
// always a placeholder ("ok", "yes", "fix"). We reject early and let the LLM
// retry with a real justification.
const RATIONALE_MIN_LEN = 20;

function validateRationale(rationale) {
  const r = typeof rationale === 'string' ? rationale.trim() : '';
  if (r.length < RATIONALE_MIN_LEN) {
    return {
      ok: false,
      error: 'rationale_too_short',
      expected_min: RATIONALE_MIN_LEN,
      got: r.length,
      message: `A rationale of at least ${RATIONALE_MIN_LEN} characters is required for this mutation. Please explain the why.`,
    };
  }
  return { ok: true, rationale: r };
}
...
async function audit(supa, { project_id, actor_id, action, entity_type, entity_id, field_name, previous_value, new_value, impact_description }) {
  try {
    ...
    if (lastErr) console.warn('[hearst-advisor-tools] audit insert failed after 3 attempts', lastErr.message);
  } catch {
  ...
  switch (name) {
    case 'get_project_state': {
      ...
      return JSON.stringify({
        _units_note: 'All _pct fields are stored as percent (0..100), e.g. debt_pct=62 means 62% leverage, opex_maintenance_pct=4 means 4% of revenue.',
        project,
    ...
    case 'get_scenario_details': {
      const { data, error } = await supa.from('hearst_scenarios').select('*').eq('id', input.scenario_id).eq('project_id', project_id).single();
      if (error) return JSON.stringify({ error: error.message });
      const projection = generateProjection(data);
      const source_score = calcSourceScore(data);
      // Pull attached sources
      const source_ids = Object.entries(data).filter(([k, v]) => k.endsWith('_source_id') && v).map(([, v]) => v);
      let attached_sources = [];
      if (source_ids.length) {
        const { data: srcs } = await supa.from('hearst_sources').select('id, metric_id, source_name, source_type, value, value_text, source_url, confidence_score').in('id', source_ids).eq('project_id', project_id);
    ...
    case 'update_scenario': {
      const rv = validateRationale(input.rationale);
      if (!rv.ok) return JSON.stringify(rv);
      ...
      const { data: before } = await supa.from('hearst_scenarios').select('*').eq('id', input.scenario_id).eq('project_id', project_id).single();
    ...
    case 'create_source': {
      ...
      await audit(supa, {
        ...
        impact_description: `${input.source_type} from ${input.source_name} — ${rv.rationale}`,
      });
    ...
    case 'attach_source_to_scenario': {
      ...
      const { data: before } = await supa.from('hearst_scenarios').select('id, project_id, ' + input.field_name).eq('id', input.scenario_id).eq('project_id', project_id).single();
      if (!before) return JSON.stringify({ error: 'scenario not found' });
      ...
      await audit(supa, {
        ...
        impact_description: `attached source to ${input.field_name} — ${rv.rationale}`,
      });
    ...
    case 'create_scenario': {
      ...
      if (input.base_scenario_id) {
        const { data: base } = await supa.from('hearst_scenarios').select('*').eq('id', input.base_scenario_id).eq('project_id', project_id).single();
        if (base) {
          // eslint-disable-next-line no-unused-vars -- strip identity/audit fields from clone
    ...
    case 'add_pipeline_prospect': {
      ...
      await audit(supa, {
        ...
        impact_description: `added ${data.prospect_type} prospect — ${rv.rationale}`,
      });
    ...
    case 'update_data_room_item': {
      ...
      const { data: before } = await supa.from('hearst_data_room').select('*').eq('id', input.item_id).eq('project_id', project_id).single();
      if (!before) return JSON.stringify({ error: 'item not found' });

      const { data, error } = await supa.from('hearst_data_room').update(updates).eq('id', input.item_id).eq('project_id', project_id).select().single();
      if (error) return JSON.stringify({ error: error.message });
      ...
      await audit(supa, {
        ...
        impact_description: `${input.notes || `status: ${before.status} → ${data.status}`} — ${rv.rationale}`,
      });
    ...
    case 'run_what_if_projection': {
      const { data: base, error } = await supa.from('hearst_scenarios').select('*').eq('id', input.base_scenario_id).eq('project_id', project_id).single();
      if (error) return JSON.stringify({ error: error.message });
    ...
    case 'compare_scenarios': {
      const { data, error } = await supa.from('hearst_scenarios').select('*').in('id', input.scenario_ids).eq('project_id', project_id);
      if (error) return JSON.stringify({ error: error.message });
    ...
    case 'generate_executive_report': {
      const { data: s, error } = await supa.from('hearst_scenarios').select('*').eq('id', input.scenario_id).eq('project_id', project_id).single();
      if (error) return JSON.stringify({ error: error.message });
      const projection = generateProjection(s);
    ...
    case 'navigate': {
      ...
      const cleaned = path.split('?')[0].split('#')[0].replace(/\/+$/, '') || '/admin/hearst';
      if (!NAV_PATH_WHITELIST.has(cleaned)) {
        return JSON.stringify({ ok: false, error: `path "${path}" is not whitelisted` });
      }
      return JSON.stringify({
        ok: true,
        queued: true,
        client_action: { type: 'navigate', path, reason: input.reason || null },
      });
    }

    case 'set_simulator_param': {
      const field = typeof input?.field === 'string' ? input.field : '';
      if (!SIM_PARAM_WHITELIST.has(field)) {
        return JSON.stringify({ ok: false, error: `field "${field}" is not allowed` });
      }
      ...
      return JSON.stringify({
        ok: true,
        queued: true,
        client_action: { type: 'set_simulator_param', field, value: input.value, why: input.why || null },
      });
    }
