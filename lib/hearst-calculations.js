...
export function generateProjection(scenario) {
  ...
  const yearly_debt_service = (debt_amount && debt_interest_rate)
    ? debt_amount * ((debt_interest_rate || 0) / 100) // interest-only; generateDebtSchedule() provides full amortization
    : null;
  ...
  // debt_pct is pure percent (0..100); divide by 100 to get fraction
...
export function foldGpuRevenue(projection, hardware_breakdown, scenario = {}, opts = {}) {
  ...
  const equity_invested = (projection.total_capex || 0) * (1 - (scenario?.debt_pct ?? 0) / 100);
