...
export function generateProjection(scenario) {
  ...
  // POST-tax equity cash flows: identical to pre-tax until the exit year, where an
  // exit capital-gains tax (A3) is netted from the terminal value. Seeded with the
  // same equity outflow at t0 (IDC included), mirroring cash_flows.
  const cash_flows_post_tax = [-(equity_invested || 0)];

  // ── A4 — Depreciation with book-value tracking (no truncation) ─────────────
  // Straight-line over the 10-year horizon on total capex (the depreciable basis).
  // Per operating year we book min(annual_depreciation, remaining_basis) so the
  // last partial year never over-depreciates, and we expose the undepreciated
  // basis left at the end of the horizon as book_value_at_exit. This feeds the A3
  // exit capital-gains computation (gain = terminal_value − book_value_at_exit).
  // Invariant: Σ depreciation booked + book_value_at_exit === total_capex.
  const total_capex_basis = total_capex ?? 0;
  const annual_depreciation = total_capex_basis / 10;
  let remaining_basis = total_capex_basis;
  // P0-1: cumulative depreciation booked THROUGH exit_year. The exit capital-gains
  // basis must be the undepreciated book value AT exit_year, not the end-of-horizon
  // residual (≈0). For exit_year<10 the asset is only partly depreciated.
  let depr_through_exit = 0;
  ...
  for (let y = 1; y <= 10; y++) {
    ...
    // A4: book depreciation only in operating years; never below remaining basis.
    const depreciation_y = operating ? Math.min(annual_depreciation, remaining_basis) : 0;
    remaining_basis -= depreciation_y;
    // P0-1: only depreciation booked in years up to and including exit_year reduces
    // the exit book-value basis used for the capital-gains computation.
    if (y <= exit_year) depr_through_exit += depreciation_y;
    ...
    // Operating-year FCFE is identical pre/post-tax (no income-tax modeled here);
    // the only post-tax divergence is the exit capital-gains tax, applied below.
    // P0-3a: cash operating taxes are zero in operations (only the exit gain is taxed),
    // so the per-year post-tax FCFE === fcf_equity_true − cash_taxes (cash_taxes = 0).
    const cash_taxes_y = 0;
    const fcf_equity_post_tax = (fcf_equity_true ?? 0) - cash_taxes_y;
    cash_flows_post_tax.push(fcf_equity_post_tax);
    ...
    years.push({
      ...
      maintenance_capex,
      depreciation: depreciation_y,              // A4: straight-line, operating years only
      operating,                                 // A8: pre-COD years (false) never book D&A/interest in the fold
      free_cash_flow: fcf_equity_true,            // real FCFE — negative allowed (drives returns)
      free_cash_flow_post_tax: fcf_equity_post_tax, // P0-3a: per-year post-tax FCFE = FCFE − cash_taxes (=FCFE in ops)
      cumulative_fcf,
  ...
  // A4: undepreciated basis remaining at the end of the 10-year horizon.
  // Invariant: Σ depreciation booked + book_value_at_exit === total_capex.
  const book_value_at_exit = remaining_basis;
  // P0-1: undepreciated book value AT exit_year — total_capex less depreciation booked
  // THROUGH exit_year. This (NOT the end-of-horizon residual) is the capital-gains basis.
  // For exit_year<10 on a 10-yr-life asset, this is ≈ (10−exit_year)/10 × total_capex
  // (≈50% at exit_year=5); for exit_year=10 it equals book_value_at_exit (≈0).
  const book_value_at_exit_basis = Math.max(0, (total_capex ?? 0) - depr_through_exit);
  ...
  // A3 — Exit capital-gains tax (POST-tax flow only). The disposition gain is the
  // sale (terminal) value above the asset's undepreciated book value at exit, taxed
  // at Qatar's 10% rate (Law 24/2018). The PRE-tax flow keeps the gross equity
  // proceeds (cash_flows); the POST-tax flow nets the tax out (cash_flows_post_tax).
  // This is a genuine equity cost at exit — not a double-tax: no income tax is
  // booked during operations, so the only tax in the model is on the realised gain.
  // P0-1: basis is the book value AT exit_year (book_value_at_exit_basis), not the
  // end-of-horizon residual — otherwise an early exit overstates the gain & exit tax.
  const capital_gain = Math.max(0, (terminal_value || 0) - book_value_at_exit_basis);
  const exit_tax = capital_gain * (TAX_DEFAULTS.capital_gains_rate_pct / 100);

  // Add equity portion of terminal value to cash flows.
  // PRE-tax: gross terminal_value_to_equity. POST-tax: same, minus exit_tax.
  if (terminal_value_to_equity != null) {
    cash_flows[exit_year] = (cash_flows[exit_year] || 0) + terminal_value_to_equity;
    cash_flows_post_tax[exit_year] = (cash_flows_post_tax[exit_year] || 0) + (terminal_value_to_equity - exit_tax);
  }
  ...
  // A3: post-tax return metrics derived from the post-tax equity cash flows.
  ...
  const moic = (equity_invested && equity_invested > 0)
    ...
    : null;
  // A3: post-tax MOIC mirrors moic but on the post-tax inflows (exit tax netted at exit).
  ...
  return {
    ...
    construction_years,
    book_value_at_exit,           // A4: undepreciated basis at end of horizon (Σ depr + this === total_capex)
    book_value_at_exit_basis,     // P0-1: undepreciated book value AT exit_year (capital-gains basis)
    terminal_tax: exit_tax,       // A3: Qatar capital-gains tax on the exit disposition gain
    irr,
    npv,
    irr_post_tax,                 // A3: IRR on post-tax equity flows (exit tax netted) — < irr when gain > 0
    npv_post_tax,                 // A3: NPV on post-tax equity flows
    payback_years,
    stabilized_ebitda,
    stabilized_revenue,
...
export function foldGpuRevenue(projection, hardware_breakdown, scenario = {}, opts = {}) {
  ...
  // (Computed before the year loop so A4 depreciation rebases on the post-fold basis.)
  const facility_capex = projection.total_capex || 0;  // pre-fold facility capex
  projection.total_capex = facility_capex + gpu_capex;

  // A4 — Rebase straight-line depreciation on the POST-fold total capex
  // (facility + GPU hardware). Track remaining book value across operating years,
  // never over-depreciating. Invariant: Σ depreciation booked + book_value_at_exit
  // === projection.total_capex (within rounding).
  const total_capex_basis_fold = projection.total_capex || 0;
  const annual_depr_fold = total_capex_basis_fold / 10;
  let remaining_basis_fold = total_capex_basis_fold;
  // P0-1: cumulative depreciation booked THROUGH exit_year on the post-fold basis.
  // Drives the exit capital-gains basis (book value AT exit_year), not the residual.
  let depr_through_exit_fold = 0;

  // Update revenue/ebitda/fcf per year with GPU revenue ramp
  projection.years.forEach((y, idx) => {
    // A8: pre-COD years (operating === false) must not book D&A, interest or fixed
    // opex. Prefer the row's own operating flag (set in generateProjection); fall
    // back to a revenue>0 heuristic for projections that predate the flag.
    const operating = y.operating ?? ((y.revenue ?? 0) > 0);
    // A4: book depreciation only in operating years; never below remaining basis.
    const depreciation_y = operating ? Math.min(annual_depr_fold, remaining_basis_fold) : 0;
    remaining_basis_fold -= depreciation_y;
    // P0-1: depreciation booked in years up to and including exit_year reduces the
    // exit book-value basis used for the post-fold capital-gains computation.
    if ((idx + 1) <= exit_year) depr_through_exit_fold += depreciation_y;
    y.depreciation = depreciation_y;
    ...
    // A8: fixed (staff) opex only once operating — facility does not exist pre-COD.
    const opex_variable = (y.revenue || 0) * opex_pct_total;
    y.opex = opex_variable + (operating ? opex_fixed : 0);
    ...
    // A8: interest (debt_service) is only carried in operating years.
    const debt_service_y = operating ? (y.debt_service || 0) : 0;
    y.free_cash_flow = y.ebitda - maint_capex - debt_service_y - refresh_this_year;
    // P0-3a: per-year post-tax FCFE. Operating cash taxes are zero (only the exit
    // gain is taxed), so free_cash_flow_post_tax === free_cash_flow − cash_taxes.
    y.cash_taxes = y.cash_taxes ?? 0;
    y.free_cash_flow_post_tax = (y.free_cash_flow ?? 0) - (y.cash_taxes ?? 0);
  });

  // A4: undepreciated basis remaining at end of horizon (post-fold basis).
  const book_value_at_exit = remaining_basis_fold;
  projection.book_value_at_exit = book_value_at_exit;
  // P0-1: undepreciated book value AT exit_year on the post-fold basis — total_capex
  // less depreciation booked THROUGH exit_year. This is the capital-gains basis.
  const book_value_at_exit_basis = Math.max(0, (projection.total_capex || 0) - depr_through_exit_fold);
  projection.book_value_at_exit_basis = book_value_at_exit_basis;

  // Equity basis: split both capex tranches by debt fraction
  const debtFrac = (scenario?.debt_pct ?? 0) / 100;
  // A1: re-include Interest During Construction (IDC) in the facility equity ticket.
  // IDC is equity-funded at close (same as generateProjection's equity_invested), so
  // a folded projection's equity_invested === facility_capex*(1-debtFrac)
  //   + gpu_capex*(1-debtFrac) + idc. Previously dropped, which understated the t0
  // equity outflow and inflated IRR/MOIC.
  const idc = projection.idc || 0;
  const facility_equity = facility_capex * (1 - debtFrac) + idc;
  ...
  // A3 — Exit capital-gains tax (POST-tax flow only). Gain = terminal (sale) value
  // above the post-fold undepreciated book value, taxed at Qatar's 10% rate. The
  // PRE-tax cashflows keep gross equity proceeds; cashflows_post_tax nets the tax.
  // Not a double-tax: no income tax is booked in operations, only this realised gain.
  // P0-1: basis is the post-fold book value AT exit_year (book_value_at_exit_basis),
  // not the end-of-horizon residual — an early exit must not overstate the gain.
  const capital_gain = Math.max(0, (projection.terminal_value || 0) - book_value_at_exit_basis);
  const exit_tax = capital_gain * (TAX_DEFAULTS.capital_gains_rate_pct / 100);
  projection.terminal_tax = exit_tax;
  ...
  for (let i = 0; i < projection.years.length; i++) {
    ...
    cashflows_post_tax.push(projection.years[i].free_cash_flow ?? 0);
  }
  ...
  // Add terminal_value_to_equity at exit_year (not gross). POST-tax nets exit_tax.
  if (terminal_value_to_equity != null) {
    cashflows[exit_year] = (cashflows[exit_year] || 0) + terminal_value_to_equity;
    cashflows_post_tax[exit_year] = (cashflows_post_tax[exit_year] || 0) + (terminal_value_to_equity - exit_tax);
  }

  projection.irr = calcIrr(cashflows);
  projection.npv = calcNpv(cashflows, discount_rate_pct / 100);
  // A3: post-tax return metrics on the post-tax equity flows.
  ...
  if (equity_invested > 0) {
    ...
    // A3: post-tax MOIC from the post-tax inflows (exit tax netted at exit).
    // P0-2: cashflows_post_tax.slice(1) INCLUDES the −gpu_equity outflow at the RFS
    // year. The pre-tax MOIC excludes it from the numerator (via the cumulative
    // algebra above, where equity_invested already books gpu_equity once at t-basis).
    // Mirror that: add gpu_equity back so the numerator counts only distributions.
    // Invariant: when operating taxes AND exit tax are both 0, moic_post_tax === moic;
    // the only gap between them is tax, never gpu_equity.
    const total_inflows_post_tax = cashflows_post_tax.slice(1).reduce((s, v) => s + v, 0);
    projection.moic_post_tax = +((total_inflows_post_tax + gpu_equity) / equity_invested).toFixed(2);
  }
 ...
 * @param {Array}  [options.cads_by_year] A5: per-year CADS (EBITDA − maintenance capex).
 *   When provided, DSCR uses CADS (the real cash available for debt service) instead
 *   of raw EBITDA. Falls back to ebitda_by_year when absent (backward compatible).
...
export function generateDebtSchedule(scenario, options = {}) {
  ...
  // A5: prefer CADS (EBITDA − maintenance capex) for DSCR when supplied.
  const cads_by_year = options.cads_by_year || null;
  const schedule = [];
  ...
  for (let y = 1; y <= debt_term_years; y++) {
    ...
    // A5: DSCR numerator = CADS when available, else EBITDA fallback (backward compatible).
    const coverage_basis = cads_by_year ? (cads_by_year[y - 1] ?? null) : (ebitda_by_year[y - 1] ?? null);
    const dscr = (coverage_basis != null && total_service > 0) ? round(coverage_basis / total_service, 2) : null;
...
export function generateWaterfall(scenario, projection) {
  ...
  const {
    debt_pct = 0, debt_interest_rate = 0, debt_term_years = 10,
  ...
  // A5: pass per-year CADS (EBITDA − maintenance capex) so DSCR reflects the real
  // cash available for debt service, not raw EBITDA.
  const cads_by_year = projection.years.map(y => (y.ebitda ?? 0) - (y.maintenance_capex ?? 0));
  const debt_sched = generateDebtSchedule(scenario, { ebitda_by_year, cads_by_year });
  ...
  // A2(ii): terminal value to equity must be NET of remaining debt principal at exit
  // (equity receives the sale value only after the lender's balloon is repaid).
  const remaining_debt_at_exit = projection.remaining_debt_at_exit
    ?? (debt_schedule[exit_year - 1]?.closing_balance
      ?? debt_schedule[debt_schedule.length - 1]?.closing_balance
      ?? 0);
  const tv_to_equity = projection.terminal_value_to_equity
    ?? Math.max(0, terminal_value - remaining_debt_at_exit);
  ...
  // A2(iii): principal amortized within the waterfall window (years 1..N). The full-
  // term summary.total_principal_paid can exceed this when debt_term_years > exit_year,
  // so we accumulate only the amortization that actually occurs before exit, then add
  // the exit balloon — avoiding double-counting principal that is still outstanding.
  let principal_amortized_in_window = 0;

  for (let y = 1; y <= projection.years.length; y++) {
    const row = projection.years[y - 1];
    // A2(i): distribute equity on CADS (EBITDA − maintenance capex), the real cash
    // available, not raw EBITDA. Maintenance capex is a true cash outflow before debt.
    ...
    const principal_repaid = debt_row?.principal || 0;
    const is_exit_year = y === exit_year;
    // A2(iii): accumulate amortized principal only up to and including the exit year.
    if (y <= exit_year) principal_amortized_in_window += principal_repaid;

    // Tier 1: lender gets debt service.
    // P2-4: with no debt (debt_service === 0), the lender receives 0 — never a negative
    // flow. min(fcf_pre_debt, 0) would push a negative number to the lender in loss years.
    // When debt exists (debt_service > 0) behavior is unchanged: min(fcf_pre_debt, service).
    let to_lender = debt_service > 0 ? Math.min(fcf_pre_debt, debt_service) : 0;
    // A2(iii): at exit, the lender's remaining principal (balloon) is repaid in full
    // from the disposition — no debt is forgiven. Push it into this year's lender flow
    // so total repaid = interest + full principal (incl. balloon).
    if (is_exit_year) to_lender += remaining_debt_at_exit;
    ...
    // A2(ii): add the NET terminal value to equity (tv_to_equity), not gross — the
    // lender's balloon was already repaid above, so equity gets only the residual.
    const residual_with_tv = remaining + (is_exit_year ? tv_to_equity : 0);
  ...
  // A2(iii): principal repaid INCLUDING the exit balloon — principal amortized within
  // the hold window (years 1..exit) plus the remaining balance retired at exit. With
  // this, principal_amortized_in_window + balloon === original principal (no debt
  // forgiven, none double-counted), and total_repaid ≈ total_interest_in_window +
  // total_principal (incl. balloon) when every year fully services its debt.
  const total_principal_amortized = round(principal_amortized_in_window, 0);
  const total_principal_incl_balloon = round(principal_amortized_in_window + remaining_debt_at_exit, 0);
  // Interest scheduled within the hold window (years 1..exit) — the only interest a
  // sold-at-exit lender can collect (summary.total_interest spans the full term).
  const total_interest_in_window = round(
    debt_schedule.slice(0, exit_year).reduce((s, r) => s + (r?.interest || 0), 0), 0);
  ...
  return {
    by_investor: {
      ...
      lender: {
        ...
        total_interest: round(debt_sched?.summary?.total_interest || 0, 0),  // full-term schedule interest
        total_interest_in_window,                              // A2(iii): interest collectible before exit
        total_principal: total_principal_amortized,            // amortized within the hold window
        total_principal_incl_balloon,                          // A2(iii): + exit balloon
        balloon_repaid_at_exit: round(remaining_debt_at_exit, 0),
    ...
    summary: {
      ...
      pref_rate: PREF_RATE,
      terminal_value,
      terminal_value_to_equity: tv_to_equity,   // A2(ii): NET TV distributed to equity
      remaining_debt_at_exit: round(remaining_debt_at_exit, 0),
    },
  };
}
...
export function generateSensitivity(scenario, paramX, paramY, steps = 5) {
  ...
  // Baseline
  // A7: cells and baseline use POST-tax IRR (irr_post_tax) — the return after the
  // exit capital-gains tax, which is the number an investor actually realizes.
  // Falls back to pre-tax irr when post-tax is unavailable (e.g. tax-free scenarios).
  const baseline_proj = generateProjection(scenario);
  const baseline = {
    irr: baseline_proj.irr_post_tax ?? baseline_proj.irr,
  ...
  const cells = y_values.map(yv =>
    x_values.map(xv => {
      ...
      const irr = proj.irr_post_tax ?? proj.irr;   // A7: post-tax IRR
