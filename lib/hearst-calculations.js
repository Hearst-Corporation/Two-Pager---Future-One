...
import { BUSINESS_MODELS, SITE_READINESS } from './hearst-constants';
...
export function generateProjection(scenario) {
  ...
  const equity_invested = total_capex != null ? total_capex * (1 - (debt_pct || 0) / 100) : null;

  const years = [];
  // t0 outflow = equity, NOT full capex
  ...
  for (let y = 1; y <= 10; y++) {
    ...
    const fcf = ebitda != null ? ebitda - maintenance_capex - (debt_service_y || 0) : null;
    // Non-recourse equity: floor cash flow at 0 (equity holders cannot be called
    // for additional capital when FCFE is negative; the lender bears that shortfall risk).
    const fcf_equity = Math.max(0, fcf ?? 0);
    cumulative_fcf += fcf_equity;
    cash_flows.push(fcf_equity);
  ...
  if (terminal_value_to_equity != null) cash_flows[exit_year] = (cash_flows[exit_year] || 0) + terminal_value_to_equity;
  ...
  // MOIC on non-recourse equity basis: sum floored distributions (mirrors IRR cash_flows)
  // cash_flows[0] = -equity_invested, cash_flows[1..N] = max(0, fcf), cash_flows[exit] += TV_to_equity
  // MOIC = total_inflows / equity_invested
  const last_cumulative_fcf = years[years.length - 1]?.cumulative_fcf ?? 0;
  const total_floored_inflows = cash_flows.slice(1).reduce((s, v) => s + v, 0); // already includes TV_to_equity
  const moic = (equity_invested && equity_invested > 0)
    ? total_floored_inflows / equity_invested
    : null;
...
export function foldGpuRevenue(projection, hardware_breakdown, scenario = {}, opts = {}) {
  ...
  if (rfs_year_n > exit_year_n) {
    // Asset is sold before GPU CAPEX hits → skip fold entirely
    return projection;
  }
  ...
  // Wave 1 (C10) — GPU refresh CAPEX disclosure (no lifecycle model yet).
  // GPU CAPEX is deployed once and never re-deployed or depreciated, while the
  // neocloud deal terms assume a ~24-month chip refresh. Disclose this so the
  // GPU-driven payback/IRR is read as an upper bound, not a steady state.
  projection.warnings = projection.warnings || [];
  projection.warnings.push('GPU refresh CAPEX not currently modeled — GPU economics assume a single hardware deployment with no ~24-month refresh cycle or residual-value loss. Treat GPU-driven payback/IRR as an upper bound.');
  ...
  // Non-recourse floor: equity holders cannot be called for additional capital (mirrors waterfall).
  const cashflows = [-facility_equity];
  for (let i = 0; i < projection.years.length; i++) {
    cashflows.push(Math.max(0, projection.years[i].free_cash_flow ?? 0));
  }
  ...
  // FIX P1 (fold): floor free_cash_flow per row (mirrors cashflows[] build above),
  // accumulate floored cumulative_fcf, and add terminal_value_to_equity to exit year row.
  projection.years.forEach(y => {
    y.free_cash_flow = Math.max(0, y.free_cash_flow ?? 0);
  });
  ...
  if (equity_invested > 0) {
    // MOIC on non-recourse equity basis: derived from final cumulative_fcf so the
    // invariant (last_cum / equity_invested + 1 == moic) holds exactly.
    // last_cum = -equity_invested + total_floored_distributions + TV_to_equity
