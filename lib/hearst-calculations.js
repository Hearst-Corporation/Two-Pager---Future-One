...
import { BUSINESS_MODELS, SITE_READINESS, GPU_REFRESH_DEFAULTS, FINANCIAL_THRESHOLDS } from './hearst-constants';
...
export function generateProjection(scenario) {
  ...
  for (let y = 1; y <= 10; y++) {
    ...
    // it to 0 erases bad years and inflates IRR/MOIC. We keep both:
    //   - fcf_equity_true : real cash flow, negative allowed → drives IRR/MOIC/payback
    //   - fcf_equity_floored : non-recourse floor (equity is never called for more
    //     capital), kept only for display of the legal cash position.
    const fcf_equity_true = ebitda != null ? ebitda - maintenance_capex - (debt_service_y || 0) : null;
    const fcf_equity_floored = Math.max(0, fcf_equity_true ?? 0);
    ...
    years.push({
      ...
      free_cash_flow_floored: fcf_equity_floored, // non-recourse legal floor (display only)
      cumulative_fcf,
