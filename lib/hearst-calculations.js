...
function calcEnergyCost({ it_mw, pue, electricity_price_mwh }) {
  if (!it_mw || !pue || electricity_price_mwh == null) return null;
  if (electricity_price_mwh === 0) return 0;
  return it_mw * pue * HOURS_PER_YEAR * electricity_price_mwh;
}

/** Occupied kW = IT MW × 1,000 × Occupancy % */
function calcOccupiedKw({ it_mw, occupancy_pct }) {
  if (!it_mw || occupancy_pct == null) return null;
  return it_mw * KW_PER_MW * (occupancy_pct / 100);
}

/** Colocation Revenue = Occupied kW × Monthly Price per kW × 12 */
function calcColoRevenue({ occupied_kw, monthly_price_kw }) {
...
function calcEbitda({ revenue, power_cost, opex }) {
...
function calcDscr({ ebitda, maintenance_capex, debt_service }) {
...
function calcTerminalValue({ exit_ebitda, exit_multiple }) {
...
function calcPayback(yearly_fcf, initial_capex) {
...
function calcTotalCapex({ total_mw, shell, mep, substation, cooling, grid, land = 0, contingency_pct = 10, liquid_premium = 0 }) {
...
export function generateProjection(scenario) {
  ...
  function getOccupancy(year) {
    const base = OCCUPANCY_RAMP[Math.min(year, 10)] || 0.95;
  ...
  for (let y = 1; y <= 10; y++) {
    ...
    const maintenance_capex = revenue ? revenue * MAINTENANCE_CAPEX_RATIO : 0;
  ...
  const stabilized_row = years.find(y => y.occupancy_pct >= OCCUPANCY_STABILIZATION_PCT) || null;
...
export function foldGpuRevenue(projection, hardware_breakdown, scenario = {}, opts = {}) {
  ...
  projection.years.forEach((y, idx) => {
    ...
    const maint_capex = (y.revenue || 0) * MAINTENANCE_CAPEX_RATIO;
  ...
  projection.stabilized_revenue = projection.years.find(y => y.occupancy_pct >= OCCUPANCY_STABILIZATION_PCT)?.revenue ?? null;
  projection.stabilized_ebitda = projection.years.find(y => y.occupancy_pct >= OCCUPANCY_STABILIZATION_PCT)?.ebitda ?? null;
  ...
  const stab_row = projection.years.find(y => y.occupancy_pct >= OCCUPANCY_STABILIZATION_PCT);
...
export function generateWaterfall(scenario, projection) {
  ...
  // Pure percent convention: debt_pct is 0..100. Projection is authoritative
  // when it carries IDC / staged hardware equity in its levered equity basis.
  ...
  const terminal_value_to_equity = projection.terminal_value_to_equity ?? terminal_value;
  ...
  for (let y = 1; y <= projection.years.length; y++) {
    ...
    const debt_payoff_at_exit = is_exit_year ? remaining_debt_at_exit : 0;
    lender_flows.push(to_lender + debt_payoff_at_exit);
    ...
    // Tier 3: residual by equity split. Equity receives terminal value net of
    // remaining debt; the debt payoff is reflected in lender_flows above.
    const residual_with_tv = remaining + (is_exit_year ? terminal_value_to_equity : 0);
