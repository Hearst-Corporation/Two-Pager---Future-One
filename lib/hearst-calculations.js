...
import { BUSINESS_MODELS } from './hearst-constants';
...
export function generateProjection(scenario) {
  ...
  function getOccupancy(year) {
    ...
    // target_occupancy_pct may arrive as percent (80) or ratio (0.80) — coerce to percent scale
    const target = target_occupancy_pct
      ? (target_occupancy_pct > 1 ? target_occupancy_pct : target_occupancy_pct * 100)
      : 90;
    return Math.min(1.0, base * (target / 90));
  }
  ...
  function getWeightedPrice(year) {
    ...
    for (const [model, pct] of Object.entries(split)) {
      ...
      if (price) {
        weighted_price += (pct / 100) * price * Math.pow(1 + annual_escalation_pct, year - 1);
        total_pct += pct / 100;
      }
    }
  ...
  // ratio 0..1 (asRatio() handles legacy 0..100 input)
  const debt_amount = total_capex ? total_capex * asRatio(debt_pct || 0) : null;
  const yearly_debt_service = (debt_amount && debt_interest_rate)
    ? debt_amount * asRatio(debt_interest_rate) // interest-only; generateDebtSchedule() provides full amortization
    : null;
  ...
  for (let y = 1; y <= 10; y++) {
    ...
    const opex_pct_total = asRatio(opex_maintenance_pct || 0) + asRatio(opex_insurance_pct || 0) + asRatio(opex_ga_pct || 0) + asRatio(opex_operator_mgmt_fee_pct || 0);
  ...
  // debt_pct uses asRatio() to handle both 0..1 and legacy 0..100 inputs
  const equity_invested = (total_capex != null && asRatio(scenario.debt_pct ?? 0) < 1)
    ? total_capex * (1 - asRatio(scenario.debt_pct ?? 0))
    : null;
 ...
 * @param {Object} opts
 * @param {number} [opts.rfs_year=3] — first year with GPU revenue (post-construction)
...
export function foldGpuRevenue(projection, hardware_breakdown, scenario = {}, opts = {}) {
  ...
  if (!hardware_breakdown?.revenue_ai_annual) return projection;
  const { rfs_year = 3, ramp_years = 2, exit_year = 10, discount_rate_pct = 10 } = opts;
  const gpu_rev_annual = hardware_breakdown.revenue_ai_annual;
  ...
  projection.years.forEach((y, idx) => {
    const yrsPostRfs = (idx + 1) - rfs_year;
    ...
    y.free_cash_flow = y.ebitda - maint_capex - (y.debt_service || 0);
  });
  ...
  // Build cashflows: year 0 = facility capex only; GPU capex deployed at rfs_year
  const cashflows = [-facility_capex];
  ...
  cashflows[rfs_idx + 1] = (cashflows[rfs_idx + 1] || 0) - gpu_capex;

  // Add terminal_value at exit_year
  if (projection.terminal_value != null) {
    cashflows[exit_year] = (cashflows[exit_year] || 0) + projection.terminal_value;
  }

  projection.irr = calcIrr(cashflows);
  projection.npv = calcNpv(cashflows, discount_rate_pct / 100);

  // Recompute cumulative_fcf with GPU capex deployed at RFS year
  let cum = -facility_capex;
  projection.years.forEach((y, i) => {
    cum += (y.free_cash_flow ?? 0);
    if (i === rfs_idx) cum -= gpu_capex;  // GPU capex deployed at RFS year
  ...
  // Recompute MOIC using post-fold equity_invested (total_capex grew by gpu_capex)
  const equity_invested = (projection.total_capex || 0) * (1 - asRatio(scenario?.debt_pct ?? 0));
  if (equity_invested > 0) {
    const last_cum = projection.years[projection.years.length - 1]?.cumulative_fcf ?? 0;
    projection.moic = +(((last_cum + (projection.terminal_value || 0)) / equity_invested)).toFixed(2);
  }
  ...
  projection.payback_years = (() => {
    let c = -facility_capex;
    for (let i = 0; i < projection.years.length; i++) {
      c += projection.years[i].free_cash_flow ?? 0;
      if (i === rfs_idx) c -= gpu_capex;
      if (c >= 0) return i + 1;
    }
    return null;
  })();
 ...
 * debt_pct and debt_interest_rate are stored as 0..1 ratios.
 *
...
export function generateDebtSchedule(scenario, options = {}) {
  const {
    ...
    capex_contingency_pct = 0.10, debt_pct = 0, debt_interest_rate,
  ...
  // ratio 0..1 (asRatio() handles legacy 0..100 input)
  const principal = total_capex * asRatio(debt_pct);
  const rate = asRatio(debt_interest_rate);
  ...
  const breach_years = schedule.filter(r => r.dscr != null && r.dscr < 1.25).map(r => r.year);
...
export function generateWaterfall(scenario, projection) {
  ...
  // asRatio() handles both 0..1 and legacy 0..100 inputs
  const total_equity = total_capex * (1 - asRatio(debt_pct));
  const total_debt = total_capex * asRatio(debt_pct);
...
const SENSITIVITY_PARAMS = {
  ...
  debt_pct:                  { label: 'Debt Leverage',        range_pct: 0.25, unit: '%', clamp: [0, 0.99] },
