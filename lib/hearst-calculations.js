...
// Ce moteur reste en production mais est jugé NON FIABLE (returns surévalués, double-count
// GPU/TV, hypothèses opaques — cf. docs/finance-engine-audit-2026-06 + docs/investment-model-migration.md).
// Remplacement prévu par lib/investment-model/ (modèle simple issu du HTML canonique Qatar).
// NE PAS étendre. NE PAS s'appuyer dessus pour de nouvelles features.
//
...
export function generateProjection(scenario) {
  ...
  const idc = (debt_amount && debt_interest_rate)
    ...
    : 0;
  if (idc > 0) {
    warnings.push(`Interest During Construction (~$${(idc / 1e6).toFixed(1)}M) is expensed in the pre-COD years (negative FCFE), not capitalized — counted once.`);
  }

  // Equity ticket (levered basis): equity investor puts in total_capex × (1 − debt%).
  // IDC is NOT added here — it is expensed as negative FCFE in the pre-COD years
  // (free_cash_flow = -debt_service for each pre-COD year). Counting it here AND in
  // the pre-COD series would double-count it and under-state IRR on greenfield deals.
  const equity_invested = total_capex != null
    ? total_capex * (1 - (debt_pct || 0) / 100)
    : null;
  ...
  for (let y = 1; y <= 10; y++) {
    ...
    // Pre-COD years (ebitda==null): IO interest still accrues and is an equity outflow.
    // Zeroing it understates the equity cost and inflates IRR for greenfield + levered.
    const fcf_equity_true = ebitda != null
      ? ebitda - maintenance_capex - (debt_service_y || 0)
      : -(debt_service_y || 0);
...
export function generateWaterfall(scenario, projection) {
  ...
  // Use equity_invested from projection (excludes IDC — IDC is expensed in pre-COD years, not capitalized).
  // Fallback to total_capex × (1 − debt%) only if projection doesn't expose it (shouldn't happen).
  const total_equity = projection.equity_invested ?? (total_capex * (1 - debt_pct / 100));
  ...
  // Terminal value at exit year — equity receives only the net (TV minus remaining debt).
  // projection.terminal_value_to_equity is already net of remaining_debt_at_exit.
  const exit_year = scenario.exit_year || 10;
  const terminal_value = projection.terminal_value || 0;
  // remaining_debt_at_exit is already net of all prior principal repayments.
  const remaining_debt_at_exit = projection.remaining_debt_at_exit ?? 0;
  // terminal_value_to_equity from the projection is already net of residual debt — add as-is at exit.
  const tv_to_equity = projection.terminal_value_to_equity ?? 0;
  ...
  // Capital-returned trackers for preferred accrual on UNRETURNED capital only.
  let hearst_capital_returned = 0;
  let brookfield_capital_returned = 0;
  let qatar_capital_returned = 0;
  ...
  for (let y = 1; y <= projection.years.length; y++) {
    ...
    // ── CANONICAL FCFE (post-tax, negative allowed — no floor) ──────────────
    // free_cash_flow_post_tax is the series that drives irr_post_tax/moic_post_tax.
    // It already accounts for: revenue − opex − capex − debt_service − taxes.
    // We do NOT recompute from EBITDA here. Negative years are capital calls (split
    // pro-rata by equity share below).
    const fcfe_post_tax = row.free_cash_flow_post_tax ?? 0;

    // ── Tier 1: lender — FULL contracted debt service (no cap on EBITDA) ────
    // The projection already deducts the FULL debt service from FCFE.
    // We reconstruct lender inflow = scheduled service + residual payoff at exit.
    const debt_row = debt_schedule[y - 1];
    const debt_service_y = debt_row?.total_service || 0;
    const lender_payoff = is_exit_year ? remaining_debt_at_exit : 0;
    lender_flows.push(debt_service_y + lender_payoff);

    // ── Available cash to equity (canonical, may be negative) ───────────────
    // tv_to_equity is already net of remaining_debt_at_exit → add directly at exit.
    let equity_cash = fcfe_post_tax + (is_exit_year ? tv_to_equity : 0);

    if (equity_cash < 0) {
      // Capital call year: split loss pro-rata by equity share (negative flows for each investor).
      hearst_flows.push(equity_cash * hearst_share);
      brookfield_flows.push(equity_cash * brookfield_share);
      qatar_flows.push(equity_cash * qatar_share);
      // Pref accrues on full unreturned capital (no cash to reduce it).
      hearst_pref_accrued += Math.max(0, hearst_equity_in - hearst_capital_returned) * PREF_RATE;
      brookfield_pref_accrued += Math.max(0, brookfield_equity_in - brookfield_capital_returned) * PREF_RATE;
      qatar_pref_accrued += Math.max(0, qatar_equity_in - qatar_capital_returned) * PREF_RATE;
      continue;
    }

    let remaining = equity_cash;

    // ── Tier 2: preferred return — accrues on UNRETURNED capital only ────────
    // Base for accrual = max(0, equity_in − capital_returned).
    // Once capital is fully returned in year k, pref stops growing in k+1.
    const hearst_pref_base = Math.max(0, hearst_equity_in - hearst_capital_returned);
    const brookfield_pref_base = Math.max(0, brookfield_equity_in - brookfield_capital_returned);
    const qatar_pref_base = Math.max(0, qatar_equity_in - qatar_capital_returned);
    hearst_pref_accrued += hearst_pref_base * PREF_RATE;
    brookfield_pref_accrued += brookfield_pref_base * PREF_RATE;
    qatar_pref_accrued += qatar_pref_base * PREF_RATE;
    const total_pref_due = hearst_pref_accrued + brookfield_pref_accrued + qatar_pref_accrued;

    // Pay pref from available cash (cannot exceed what's owed).
    ...
    // ── Tier 3: residual split by equity share ───────────────────────────────
    // Residual is remaining cash after pref; no terminal value added here
    // (tv_to_equity was already folded into equity_cash above).
    const hearst_carry = remaining * hearst_share;
    const brookfield_carry = remaining * brookfield_share;
    const qatar_carry = remaining * qatar_share;

    // Track capital returned for pref-accrual base in future years.
    // Carry reduces unreturned principal.
    hearst_capital_returned = Math.min(hearst_equity_in, hearst_capital_returned + hearst_carry);
    brookfield_capital_returned = Math.min(brookfield_equity_in, brookfield_capital_returned + brookfield_carry);
    qatar_capital_returned = Math.min(qatar_equity_in, qatar_capital_returned + qatar_carry);
