...
export function generateProjection(scenario) {
  ...
  // debt_pct stored as 0..100 (e.g. 65 = 65%), debt_interest_rate as 0..100 (e.g. 5.5 = 5.5%)
  const debt_amount = total_capex ? total_capex * ((debt_pct || 0) / 100) : null;
  const yearly_debt_service = (debt_amount && debt_interest_rate)
    ? debt_amount * (debt_interest_rate / 100) // interest-only; generateDebtSchedule() provides full amortization
    : null;
  ...
  // debt_pct is in 0..100 scale; equity = (1 - debt_pct/100) × total_capex
