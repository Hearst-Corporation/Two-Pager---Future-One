...
export function generateWaterfall(scenario, projection) {
  ...
  // Using gross terminal_value here would double-count the lender's principal repayment
  // in the equity distribution pool. projection.terminal_value_to_equity is already
  // net of remaining_debt_at_exit (computed in generateProjection and foldGpuRevenue).
  ...
  for (let y = 1; y <= projection.years.length; y++) {
    ...
    // Use terminal_value_to_equity (net of remaining debt) not gross terminal_value —
    // the lender's principal repayment at exit must not flow into the equity pool.
