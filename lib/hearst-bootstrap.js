...
//
// La librairie PUBLIC_SOURCES_LIBRARY (lib/oracle-intelligence/source-library.js) contient
...
// extraSources (DB rows flagged used_in_model=true) override the static-library
// median for their metric_id — geography filter bypassed for those metrics.
...
function filterByGeography(sources, geography) {
  const geo = (geography || DEFAULT_GEOGRAPHY).toLowerCase();
  if (geo === 'global') return sources;
  return sources.filter(s => {
    const sgeo = (s.geography || '').toLowerCase();
    if (geo === DEFAULT_GEOGRAPHY) {
    ...
    if (geo === 'mena' || geo === 'gcc') {
      // GCC == regional aggregate: any Gulf/MENA/member-state comp.
      return sgeo.includes('mena') || sgeo.includes('gulf') || sgeo.includes('gcc')
        || sgeo.includes('qatar') || sgeo.includes('uae') || sgeo.includes('saudi') || sgeo.includes('ksa');
    }
    if (geo === 'uae' || geo === 'ksa') {
      // No invented per-country dataset (P9): use the country's own comps where
      // they exist, else fall back to regional MENA/Gulf — never fabricated.
      if (sgeo.includes(geo)) return true;
      if (geo === 'ksa' && sgeo.includes('saudi')) return true;
      return sgeo.includes('mena') || sgeo.includes('gulf');
    }
 ...
 * @param {Array}  [opts.extraSources=[]]    DB rows (used_in_model=true) — override static library per metric
...
export function bootstrapScenarioFromSources(opts = {}) {
  ...
  const scenario = {
    ...
    // Equity split par défaut Brookfield + Qatar narrative (pure percent, sum = 100)
    equity_hearst_pct: 20,
    equity_brookfield_pct: 55,
    equity_qatar_pct: 25,
  ...
  // Build DB override index: metric_id → [db rows with value != null].
  const dbByMetric = new Map();
  for (const s of extraSources) {
    if (!s || !s.metric_id || s.value == null) continue;
    const list = dbByMetric.get(s.metric_id) || [];
    list.push(s);
    dbByMetric.set(s.metric_id, list);
  }

  // Agréger PUBLIC_SOURCES_LIBRARY par metric_id.
  ...
  // DB overrides replace the static group for their metric_id.
  for (const [metric_id, dbList] of dbByMetric.entries()) {
    grouped.set(metric_id, dbList);
  }
  ...
  for (const [metric_id, sources] of grouped.entries()) {
    ...
    let filtered;
    if (dbByMetric.has(metric_id)) {
      // DB-overridden metric: bypass geo filter — user explicitly chose these sources.
      filtered = sources;
    } else {
      filtered = filterByGeography(sources, geography);
      // Fallback en cascade : Qatar → MENA → Global (toutes confidences)
      if (filtered.length === 0 && (geography || '').toLowerCase() === DEFAULT_GEOGRAPHY) {
