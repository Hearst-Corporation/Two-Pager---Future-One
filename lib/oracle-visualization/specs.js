...
function build2DDiagramSpec(simulation) {
  ...
  // ------------------------------------------------------------------
  const ai_label = ai_pct > 0 ? ' · GB200 cluster' : '';
  const summary = `${total_mw} MW · ${classic_pct}% classic / ${liquid_pct}% liquid / ${ai_pct}% AI${ai_label}`;

  // ------------------------------------------------------------------
  // Legend — include density hint
  // ------------------------------------------------------------------
  const legend = [
    { kind: 'classic', label: `Classic Compute (~${DENSITY_KW_PER_RACK.classic} kW/rack)`,    color_token: KIND_COLOR.classic },
    { kind: 'liquid',  label: `Liquid Cooling HPC (~${DENSITY_KW_PER_RACK.liquid} kW/rack)`,  color_token: KIND_COLOR.liquid  },
    { kind: 'ai',      label: `AI / GPU Cluster (~${DENSITY_KW_PER_RACK.ai} kW/rack)`,        color_token: KIND_COLOR.ai      },
  ];
 ...
 */
function buildTopologySpec(_simulation) {
