...
// Straight-line depreciation over a 15-yr blended life (see depreciable_life_years
// below for the Qatar GTA Art. 16 + REIT 10-K sourcing) — NO accelerated schedule,
// NO loss carryforwards, NO deferred tax assets, NO jurisdiction engine.
export const TAX_DEFAULTS = {
  income_tax_rate_pct:   10,    // Qatar corporate income tax (Law No. 24 of 2018)
  // Blended straight-line life of a full data center, sourced to Qatar's own
  // statutory schedule (Law 24/2018 + Cabinet Decision 39/2019, Executive
  // Regulations Art. 16) cross-checked against REIT 10-K useful lives:
  //   • building shell / durable constructions  5%/yr  = 20 yr (GTA Art. 16)
  //   • electricity prod./transmission/distrib. 5%/yr  = 20 yr (GTA Art. 16)
  //   • machinery, electrical appliances, MEP   20%/yr =  5 yr (GTA Art. 16)
  //   • air conditioners / cooling              25%/yr =  4 yr (GTA Art. 16)
  //   • computers / IT                          33.33% =  3 yr (GTA Art. 16)
  // Capex-weighted blend (shell ~37% @20yr + MEP/cooling/elec ~56% @5-20yr +
  // grid ~7% @20yr) lands at ~15 yr — the defensible mid-point of the 10-20 yr
  // band (Digital Realty 10-K blends ~21yr building-heavy; Equinix ~15-17yr
  // equipment-heavy). 10 yr was too aggressive (overstated D&A shield); 20 yr
  // is building-only and ignores the equipment-heavy MEP load.
  depreciable_life_years: 15,   // straight-line, capex-weighted blend (Qatar GTA Art. 16 + REIT 10-K)
};
...
// icon = lucide-react component name (not an emoji) — resolve to a glyph when the
// data-room UI renders these categories.
