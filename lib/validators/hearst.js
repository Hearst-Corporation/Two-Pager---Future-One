...
//   - Every UUID is .uuid().
//   - Enums come from lib/hearst-constants.js (kept here as small literal arrays).
//
...
// Mirrors SUPPORTED_AUDIENCES from lib/oracle-explainability.js.
const SUPPORTED_AUDIENCE_VALUES = ['beginner', 'investor', 'minister', 'operator'];
...
// Percent100 — ALL _pct fields are stored as 0..100 (pure percent convention).
// Examples: debt_pct=62 → 62% leverage, capex_contingency_pct=10 → 10% contingency,
// equity_hearst_pct=20 → Hearst holds 20% of equity, annual_escalation_pct=2 → 2%/yr.
// Callers divide by 100 at each consumption site — no ratio (0..1) fields remain.
const Percent100 = z.number().finite().min(0).max(100);
// IntegerPct — 0..100 integer (e.g. probability_pct).
...
// Mirrors scenario writable fields from the API contract, plus the
...
const ScenarioWritableFields = {
  ...
  capex_contingency_pct: Percent100.optional().nullable(), // realistic range ~5–30%; 0..100 allows stress scenarios
  ...
  opex_maintenance_pct: Percent100.optional().nullable(),
  opex_staff_annual_musd: z.number().finite().min(0).max(1000).optional().nullable(),
  opex_insurance_pct: Percent100.optional().nullable(),
  opex_ga_pct: Percent100.optional().nullable(),
  opex_operator_mgmt_fee_pct: Percent100.optional().nullable(),
  ...
  annual_escalation_pct: z.number().finite().min(-100).max(100).optional().nullable(), // realistic ~2–3%/yr; ±100 allows deflation/stress scenarios

  // Capital structure
  equity_hearst_pct: Percent100.optional().nullable(),
  equity_brookfield_pct: Percent100.optional().nullable(),
  equity_qatar_pct: Percent100.optional().nullable(),
  debt_pct: Percent100.optional().nullable(),
  debt_interest_rate: z.number().finite().min(-1).max(50).optional().nullable(), // realistic max ~30%; 50 allows stress scenarios
...
export const SimulateRequestSchema = z.object({
  ...
  // Critical Deliverables P9 — GCC coverage. uae/ksa/gcc resolve to regional
  // (MENA/Gulf) comps in the bootstrap; no invented per-country data.
  geography: z.enum(['qatar', 'uae', 'ksa', 'gcc', 'mena', 'global']).optional(),
...
// ────────────────────────────────────────────────────────────────────────
// 7. Strategic Memo Generation
//    POST /api/admin/hearst/strategic-memo
// ────────────────────────────────────────────────────────────────────────

// Loose record type for arbitrary nested JSON objects, capped at
// a reasonable number of string keys per level to limit payload size.
const LooseRecord = z.record(z.string().max(128), z.unknown());

// Exported so a regression test can assert directly that the Results page's
// whitelisted memo payload (buildMemoPayloadFromSimResult in lib/hearst-memo-payload.js)
// satisfies this strict schema. The .strict() contract is intentionally unchanged.
export const MemoPayloadSchema = z.object({
  scenario: LooseRecord.optional(),
  projection: LooseRecord.optional(),
  hardware_breakdown: LooseRecord.optional(),
  archetype_outcome: z.object({ id: z.string().max(64) }).passthrough().optional(),
}).strict().refine(
  (v) => Boolean(v.scenario) || Boolean(v.projection),
  { message: 'payload.scenario or payload.projection is required', path: ['scenario'] },
);

const MemoOracleContextSchema = z.object({
  stakeholder: z.string().max(120).optional(),
  region: z.string().max(120).optional(),
  overlays: z.array(z.string().max(120)).max(20).optional(),
}).strict();

export const MemoGenerateSchema = z.object({
  project_id: Uuid.optional().nullable(),
  scenario_id: Uuid.optional().nullable(),
  title: z.string().max(200).optional(),
  audience: z.enum(SUPPORTED_AUDIENCE_VALUES).optional(),
  user_question: z.string().max(2000).optional(),
  oracle: MemoOracleContextSchema.optional(),
