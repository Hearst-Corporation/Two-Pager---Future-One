...
export const DEAL_ARCHETYPES = [
  ...
  },
  {
    ...
    brand_premium_pct: 0.15, // 15% premium sur prix de référence (sovereign tier)
...
export function applyArchetype(scenario, archetype) {
  ...
  // debt_interest_rate is stored as 0..100 (percent), e.g. 5.5 = 5.5%.
  // 100 bps = 1.0 percentage point, so delta = bps / 100.
  if (archetype.debt_rate_delta_bps != null && s.debt_interest_rate != null) {
    const delta_pct = archetype.debt_rate_delta_bps / 100; // bps → percentage points (0..100 scale)
    s.debt_interest_rate = Math.max(0, s.debt_interest_rate + delta_pct);
  }
