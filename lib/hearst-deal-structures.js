...
export const DEAL_ARCHETYPES = [
  ...
  },
  {
    ...
    real_comp: 'Multiple GCC government DCs use Schneider/Vertiv on O&M-only contracts.',
  },
  {
    id: 'white_label',
    code: 'D',
  ...
  },
  {
    ...
    real_comp: 'UAE G42 national AI ($1.5B Microsoft 2024); Saudi HUMAIN PIF (2024).',
  },
];
...
export function projectArchetype(scenario, archetype) {
  ...
  if (archetype.compute_as === 'one_time_sale') {
    ...
    // A6: MOIC numerator = sum of all interim distributions (equity_cash_flows[1..]),
    // which already nets the interim debt-service outflows and the final sale proceeds.
    // This puts MOIC on the SAME basis as IRR (both consume equity_cash_flows), instead
    // of crediting gross sale proceeds while ignoring the interim debt drag.
    const total_distributions_to_equity = equity_cash_flows.slice(1).reduce((s, v) => s + v, 0);
