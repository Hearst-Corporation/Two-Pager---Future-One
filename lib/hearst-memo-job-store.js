...
//   Le fetch /strategic-memo prend 30-90s (cascade Hypercli). Si la modale est
...
//     result, error, model_attempts: ['kimi-k2.6', …] }
...
/**
 * Helper UI : stage actuel estimé selon le temps écoulé.
 * Heuristique : la cascade Hypercli retry après ~25s par modèle.
 * C'est cosmétique (pas de vrai signal du serveur), mais donne au user
 * une idée de progression.
 */
export function estimateCurrentStage(elapsedMs) {
  const s = Math.floor((elapsedMs ?? 0) / 1000);
  if (s < 25) return CASCADE[0];
  if (s < 50) return CASCADE[1];
  if (s < 75) return CASCADE[2];
  return CASCADE[3];
}
