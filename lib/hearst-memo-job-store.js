...
//   Le fetch /strategic-memo prend 30-90s (OpenAI GPT-4.1). Si la modale est
...
//     result, error, cascade: ['gpt-4.1'] }

import { useSyncExternalStore } from 'react';
import { LLM_MEMO_CASCADE, MEMO_CLIENT_TIMEOUT_MS as _MEMO_CLIENT_TIMEOUT_MS } from './constants';

const CASCADE = [...LLM_MEMO_CASCADE];
export const MEMO_CLIENT_TIMEOUT_MS = _MEMO_CLIENT_TIMEOUT_MS;

// The only keys MemoPayloadSchema (lib/validators/hearst) accepts. The raw
// POST /api/admin/hearst/simulate response carries ~10 keys (waterfall,
// debt_schedule, source_map, confidence_score, derived, solver, …); the schema
// is .strict() and rejects any extra key, so posting the full response 400s.
const MEMO_PAYLOAD_KEYS = ['scenario', 'projection', 'hardware_breakdown', 'archetype_outcome'];

/**
 * Whitelist the raw /simulate response down to the strict memo payload shape.
 * Pure: copies only the four allowed keys that are present, never fabricates a
 * key, and does NOT mutate the input.
 *
 * @param {object|null|undefined} simResult  raw /api/admin/hearst/simulate response
 * @returns {{ scenario?: object, projection?: object, hardware_breakdown?: object, archetype_outcome?: object }}
 */
export function buildMemoPayloadFromSimResult(simResult) {
  const src = simResult || {};
  const payload = {};
  for (const key of MEMO_PAYLOAD_KEYS) {
