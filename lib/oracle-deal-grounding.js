...
export function buildDealGroundingBlock(deal) {
  ...
  // Core return metrics from the engine
  const irrStr     = p.irr != null         ? fmtPctFromRatio(p.irr)        : '—';
  const moicStr    = p.moic != null         ? fmtX(p.moic)                  : '—';
  const npvStr     = p.npv != null          ? fmtUSD(p.npv)                 : '—';
  ...
  lines.push(
    `Engine (levered-equity): IRR ${irrStr} · MOIC ${moicStr} · NPV ${npvStr} · Payback ${paybackStr} · DSCR ${dscrStr}`,
  );
