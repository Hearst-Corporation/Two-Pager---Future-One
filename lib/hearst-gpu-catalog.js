...
// Source signalée du double-count GPU/TV à l'audit moteur. Voir docs/investment-model-migration.md.
//
...
export const GPU_CATALOG = [
  {
    ...
    notes: 'Hopper. Reference rack ≈ 6 kW (8× SXM5 + host).',
    rack_scale: false,
  },
  {
    id: 'h200_sxm5',
    ...
    notes: 'Hopper refresh — bumped HBM to 141 GB for long-context inference.',
    rack_scale: false,
  },
  {
    id: 'gb200_nvl72',
    ...
    notes: 'Rack-scale Blackwell. 132 kW/rack (NVIDIA spec) — REQUIRES liquid cooling.',
    rack_scale: true,
  },
  {
    id: 'mi300x',
    ...
    notes: 'AMD Instinct — 192 GB HBM3, cost-optimised inference.',
    rack_scale: false,
  },
];

/** Map id → row pour lookup O(1). */
export const GPU_BY_ID = Object.fromEntries(GPU_CATALOG.map(g => [g.id, g]));
 ...
 */
export function calcRackPower(gpu) {
  if (!gpu) return 0;
 ...
 * @returns {number} USD
 */
export function calcGpuCapex(num_racks, gpu) {
 ...
 *
 * `gpu_hour_price` est TOUJOURS en $/GPU-heure pour TOUS les SKUs, y compris
 * GB200 NVL72. Pour NVL72, density_per_rack = 72 — donc total_gpus = num_racks × 72,
 * cohérent avec calcGpuCapex (msrp_usd par rack de 72 GPU, même dénominateur).
 *
 * @param {number} num_racks
 * @param {GpuCatalogRow} gpu
 ...
 */
export function calcGpuAnnualRevenue(num_racks, gpu, utilization_pct, gpu_hour_price) {
  if (!gpu || !num_racks || num_racks <= 0) return 0;
...
/**
 * Estime le nombre de racks pour atteindre `mw_target` à puissance donnée.
 *
 * @param {number} mw_target
 * @param {GpuCatalogRow} gpu
 * @returns {number} number of racks (entier arrondi inférieur)
 */
export function calcNumRacksForMw(mw_target, gpu) {
  if (!gpu || !mw_target || mw_target <= 0) return 0;
  const rack_kw = calcRackPower(gpu);
  if (rack_kw <= 0) return 0;
  return Math.floor((mw_target * 1000) / rack_kw);
}
...
/**
 * Prix GPU-hour de référence par SKU (USD).
 * Source : Lambda public pricing (H100=2.49) + analogie SemiAnalysis.
 */
export const REFERENCE_GPU_HOUR_PRICES = {
  h100_sxm5: 2.49,
  h200_sxm5: 2.99,
  gb200_nvl72: 5.00,
  mi300x: 1.89,
...
/**
 * Fusionne catalogue local (fallback) et catalogue Supabase (priorité).
 * Le catalogue local sert de secours quand la DB n'est pas seedée.
 *
 * @param {GpuCatalogRow[]} local
 * @param {GpuCatalogRow[]} remote
 * @returns {GpuCatalogRow[]}
 */
export function mergeCatalog(local, remote) {
  if (!Array.isArray(remote) || remote.length === 0) return local;
  // Toujours utiliser le slug du sku comme clé de dédup — l'UUID DB ne matchera
  // jamais l'id slug local et créerait des doublons.
  const slug = (sku) => String(sku || '').toLowerCase().replace(/[^a-z0-9]+/g, '_');
  const byKey = new Map(local.map(g => [slug(g.sku || g.id), g]));
  for (const row of remote) {
    if (!row || !row.sku) continue;
    const key = slug(row.sku);
    // DB priority — preserve DB id (uuid) but use slug as merge key.
    byKey.set(key, { ...row, id: row.id || key });
  }
