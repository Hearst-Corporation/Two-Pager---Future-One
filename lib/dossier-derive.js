...
const HURDLE_IRR    = 0.10;   // engine discount-rate default — IRR below this is NPV-negative
const MIN_MOIC      = 1.0;    // below 1× = capital destruction
const DSCR_COVENANT = 1.25;   // matches generateDebtSchedule breach threshold
...
export function deriveVerdict(memo) {
  ...
  const moic    = num(proj.moic);
  const npv     = num(proj.npv);
