...
export function deriveVerdict(memo) {
  ...
  const hasCoreReturns = [
    proj.irr_post_tax ?? proj.irr,
    proj.moic_post_tax ?? proj.moic,
    proj.npv_post_tax ?? proj.npv,
  ].some(v => v != null);
  ...
  // irr/moic/npv are already resolved to post-tax-with-fallback above.
  ...
  const reviewDrivers = [];
  if (irr != null && irr < FINANCIAL_THRESHOLDS.ic_hurdle_pct / 100) reviewDrivers.push(`IRR ${(irr * 100).toFixed(1)}% below ${FINANCIAL_THRESHOLDS.ic_hurdle_pct}% IC hurdle`);
...
export function deriveKpis(memo) {
  ...
  const cards = [
    {
      key: 'irr', label: 'IRR',
      value: fmtPct(proj.irr_post_tax ?? proj.irr) || rawFor(/\birr\b/),
      sub: 'Levered post-tax', confidence: confFor(/\birr\b/),
    },
    {
      key: 'moic', label: 'MOIC',
      value: fmtX(proj.moic_post_tax ?? proj.moic) || rawFor(/\bmoic\b|multiple/),
      sub: 'Equity multiple post-tax', confidence: confFor(/\bmoic\b/),
    },
    {
      key: 'npv', label: 'NPV',
      value: fmtUsd(proj.npv_post_tax ?? proj.npv) || rawFor(/\bnpv\b/),
      sub: 'Net present value post-tax', confidence: confFor(/\bnpv\b/),
    },
    {
