...
export function solveForTargetIrr(scenario, target_irr, lever, opts = {}) {
  ...
  const evalAt = (v) => {
    ...
    // Use post-tax IRR when available (matches what the board displays); fall back to pre-tax.
    return { scenario: s, projection: p, irr: p?.irr_post_tax ?? p?.irr };
  };
  ...
  if (fLow * fHigh > 0) {
    ...
    return {
      ...
      diagnostic: `Target post-tax IRR ${(target_irr * 100).toFixed(1)}% out of range [${(evalLow.irr * 100).toFixed(1)}%, ${(evalHigh.irr * 100).toFixed(1)}%] for lever ${lever}.`,
    };
  }
  ...
  return {
    ...
    projection: evalMid.projection,
    achieved_irr: evalMid.irr,
    diagnostic: `Did not converge to ±${tolerance} within ${max_iterations} iterations. Closest post-tax IRR = ${(evalMid.irr * 100).toFixed(2)}%.`,
  };
}
