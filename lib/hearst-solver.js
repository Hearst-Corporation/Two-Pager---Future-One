 ...
 */
function mwFromCapital(capital_usd, scenario_defaults = {}) {
 ...
 */
function solveForTargetIrr(scenario, target_irr, lever, opts = {}) {
  const {
    max_iterations = 25,
    tolerance = 0.001,
 ...
 */
function solveMwForCapital(budget, totalCapexForMw, opts = {}) {
...
export function solveScenarioForMode(input_mode, input_value, defaults = {}, opts = {}) {
  ...
  if (input_mode === 'target_irr_first') {
    const target_irr = (input_value?.target_irr_pct ?? FINANCIAL_THRESHOLDS.ic_hurdle_pct) / 100;
