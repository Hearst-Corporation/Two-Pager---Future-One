...
export function fitColor(fit) {
  // Monochrome bordeaux ramp (faint → strong) — keeps the matrix on the Hearst
  // charte instead of the old blue/green heatmap, and raises the low tiers so a
  // weak match stays distinguishable from an empty (—) cell.
  if (!fit || fit <= 0) return 'transparent';
  if (fit === 1) return 'color-mix(in oklch, var(--cp-accent-maroon) 12%, transparent)';
  if (fit === 2) return 'color-mix(in oklch, var(--cp-accent-maroon) 24%, transparent)';
  if (fit === 3) return 'color-mix(in oklch, var(--cp-accent-maroon) 40%, transparent)';
  if (fit === 4) return 'color-mix(in oklch, var(--cp-accent-maroon) 60%, transparent)';
  return 'color-mix(in oklch, var(--cp-accent) 80%, transparent)';
}
