...
const ORACLE_CONSTITUTION = {
  ...
  regional_contexts: {
    qatar: {
      ...
      regulatory_notes: [
        ...
        'Qatar AI Strategy launched 2024 (~$2.5B/5y program); Vision 2030 knowledge-economy alignment.',
      ],
    },
    saudi_arabia: {
  ...
  output_framework: {
    ...
    rules: [
      ...
      'Cite a source only when it is explicitly present in the provided context (deal grounding, user message, or injected corpus). Never invent, guess, or fabricate a source identifier, filing number, date, or URL. If no verifiable source is available for a claim, mark it [no verifiable source in context — treat as LOW VISIBILITY] instead of citing a placeholder.',
...
const DEFAULT_CTX = {
  ...
  language: 'en',       // 'auto' | 'fr' | 'en' — forced EN: Oracle is an English-language product; callers can pass 'fr' explicitly if needed
  surface: 'chat', // identifies the calling endpoint
};
 ...
 */
function resolveOracleContext(ctx = {}) {
 ...
 * @param {string} [ctx.surface]     — identifies caller (chat | advisor)
...
export function buildOracleSystemPrompt(ctx = {}) {
  ...
  // In concise mode: summary line only (full benchmarks available on request).
  if (r.brevity === 'concise') {
    lines.push('## Operational realism — flag proactively when scenario violates construction/supply-chain/grid/cooling benchmarks. Use reality-check phrases ("Theoretically sound, but operationally unrealistic before [year] given [constraint]."). Full benchmarks available on request.');
    lines.push('');
  } else {
  ...
  // 8 — Brevity
  if (r.brevity === 'concise') {
    lines.push('Brevity : concise. Default to ≤ 150 words unless the user explicitly asks for depth.');
  ...
  // 10 — Surface + product context (scoring models retired from prompt — data-only, Sprint 4)
...
export function inferOracleContextFromPath(pathname = '') {
  if (pathname.startsWith('/admin/hearst')) {
    return {
      stakeholder: 'investor',      // audience = Investment Committee, lens IRR/MOIC/NPV
