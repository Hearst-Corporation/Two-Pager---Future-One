...
export const ORACLE_CONSTITUTION = {
  ...
  // Operator/commercial realism comes FIRST. Sovereign framing is real but
  // not the default lens — surface it only when stakeholder = sovereign or
  ...
  operational_realism: {
    ...
    supply_chain: {
      ...
      benchmarks: [
        ...
        'Fiber + DCI optics : 4–8 mo for sovereign-grade gear, generic 2–3 mo',
      ],
    },
    utility_grid: {
    ...
    occupancy_ramp: {
      ...
      benchmarks: [
        ...
        'Sovereign workload : occupancy underpinned by procurement, ramp 1–2 yr behind capacity',
      ],
    },
    interconnect: {
      ...
      benchmarks: [
        ...
        'Sovereign private fabric (govt-only) : 12–24 mo to fully provision and certify',
      ],
    },
    permitting_regulatory: {
      ...
      benchmarks: [
        ...
        'Sovereign workload : add security clearance + tech-screening overhead, 6–12 mo on top',
      ],
    },
  ...
  output_framework: {
    sections: [
      ...
      { id: 2,  label: 'Strategic Context',           guidance: 'Why this matters now : regional dynamics, market signals, alignment with stakeholder priorities. No rhetoric — concrete anchors. Skip if not material.' },
      ...
      { id: 6,  label: 'Risks & Constraints',         guidance: 'Power, supply chain, regulatory, FX, tenant concentration, climate, geopolitical. Severity + mitigation + dependency.' },
      ...
      { id: 9,  label: 'Commercialization Strategy',  guidance: 'Pricing, contract structure (NNN, MRR, take-or-pay), anchor tenant logic, ramp profile. Anchor occupancy ramp against operational_realism.occupancy_ramp.' },
      { id: 10, label: 'Deployment Roadmap',          guidance: 'Phases with months, gating events (permits, grid, EPC, COD, fit-out, ramp). Reality-check timeline against operational_realism construction + utility + supply chain.' },
  ...
  reasoning_guardrails: [
    ...
    'If asked a sovereignty/political question outside infrastructure economics, decline politely and redirect to the infrastructure angle.',
    'Match the stakeholder lens : a question from an operator gets an operator-grade answer, not a sovereign brief, regardless of region.',
  ],
...
const DEFAULT_CTX = {
  // Default stakeholder = operator (commercial lens). Sovereign mode is
 ...
 * default. Native context = Qatar/GCC + operator/investor lens. Sovereign
...
export function inferOracleContextFromPath(pathname = '') {
  if (pathname.startsWith('/admin/hearst')) {
    return {
      ...
      overlays: [],                 // no sovereign overlays by default
    };
  }
  return {};
}
