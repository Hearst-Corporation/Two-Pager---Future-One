...
import { resolveSourceLabel } from './citation-resolver';
 ...
 */
function buildMemoMarkdown(memo, meta = {}) {
  ...
  if (Array.isArray(memo.intelligence_sources) && memo.intelligence_sources.length) {
    L.push('## Sources');
    memo.intelligence_sources.forEach(s => {
      const src = resolveSourceLabel(s);
      const label = src.url ? `[${src.label}](${src.url})` : src.label;
      L.push(`- ${label} — ${s.used_for} (${s.trust})`);
    });
    L.push('');
  }
  L.push('---');
