...
export function serializeStateToUrl(state) {
  ...
  // NOTE: `viz` / `active_viz` is intentionally NOT serialized here.
  // `serializeStateToUrl` is called only from the simulator config page,
  // where `viz` is not a valid tab. The `?viz=` deep-link is only meaningful
  // on /results, which reads it directly from window.location.search.
