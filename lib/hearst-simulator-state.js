...
import { DEFAULT_HARDWARE_MIX } from './hearst-config-presets';
...
export const INITIAL_STATE = {
  ...
  // Hardware mix — default = mixed preset (highlighted on load)
  hardware_mix: {
...
export function serializeStateToUrl(state) {
  ...
  // `?viz=` is only read on /simulator/results — not serialized from the config page.
  return params.toString();
}

const VALID_MODES = new Set(['mw_first', 'capital_first', 'target_irr_first']);
const VALID_LEVERS = new Set(['equity_pct', 'debt_rate', 'exit_cap_rate', 'exit_year', 'pricing']);
...
/**
 * Safely parse a URL numeric param. Unlike `Number(x) || fallback`, this
 * correctly handles zero values (e.g. ?mw=0 → 0, not fallback).
 */
...
export function parseStateFromUrl(searchParams) {
  ...
  // Validate mode — ignore unknown values
  if (rawMode != null) {
    if (VALID_MODES.has(rawMode)) next.mode = rawMode;
    // else: unknown mode — omit, caller falls back to INITIAL_STATE.mode
  }

  const mode = next.mode ?? INITIAL_STATE.mode;

  if (mode === 'capital_first' && searchParams.get('cap') != null) {
    next.capital_usd = parseNum(searchParams.get('cap'), INITIAL_STATE.capital_usd);
  }
  if (mode === 'mw_first' && searchParams.get('mw') != null) {
    next.total_mw = parseNum(searchParams.get('mw'), INITIAL_STATE.total_mw);
  }
  if (mode === 'target_irr_first') {
    if (searchParams.get('irr') != null) {
      next.target_irr_pct = parseNum(searchParams.get('irr'), INITIAL_STATE.target_irr_pct);
    }
    const rawLever = searchParams.get('lever');
    if (rawLever != null && VALID_LEVERS.has(rawLever)) {
      next.target_irr_lever = rawLever;
    }
    // else: invalid lever — omit
  }

  const rawArch = searchParams.get('arch');
  if (rawArch != null && VALID_ARCHETYPES.has(rawArch)) {
    next.primary_archetype_id = rawArch;
  }
  // else: unknown archetype — omit

  if (searchParams.get('elec') != null) {
    next.electricity_price_mwh = parseNum(searchParams.get('elec'), INITIAL_STATE.electricity_price_mwh);
  }
  // `?viz=` is read only on /simulator/results — not hydrated on the config page.
