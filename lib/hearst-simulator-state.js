...
import { DEFAULT_CASE_ID, getCaseOverrides } from './hearst-scenario-cases';
...
export const INITIAL_STATE = {
  ...
  // Cas d'investissement : conservative | base | upside.
  // Pilote scenario_overrides (cf. lib/hearst-scenario-cases.js). Base = bootstrap
  // sourcé vivant (overrides = {}).
  scenario_case: DEFAULT_CASE_ID,
...
export function simulatorReducer(state, action) {
  switch (action.type) {
    ...
    case ACTIONS.SET_CASE:
      return { ...state, scenario_case: action.value };
...
export function buildSimulatePayload(state) {
  ...
  // Investment case → scenario_overrides (merged over the sourced bootstrap by the
  // /simulate route). Base ⇒ {} (the live bootstrap stays the source of truth).
  const scenario_overrides = getCaseOverrides(state.scenario_case);
  ...
  // Only attach when non-empty — keeps Base payloads identical to the legacy shape
  // (and avoids a redundant key in the simKey memo that drives /simulate).
...
export function serializeStateToUrl(state) {
  ...
  if (state.scenario_case && state.scenario_case !== DEFAULT_CASE_ID) params.set('case', state.scenario_case);
...
export function parseStateFromUrl(searchParams) {
  ...
  if (searchParams.get('case')) next.scenario_case = searchParams.get('case');
  return next;
}
