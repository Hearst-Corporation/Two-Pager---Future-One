...
//
// Server-side fallback ("voie b") for cockpit-chat grounding.
//
...
export async function resolveActiveDeal() {
  ...
  // 3. Run facility projection — archetype-aware so the advisor sees the same
  //    numbers that the live /simulate pipeline (which uses projectArchetype) produced.
  //    Falls back to generateProjection when archetype_id is absent/unknown.
  const archetype = DEAL_ARCHETYPES.find(a => a.id === scenarioRow.archetype_id);
  const projection = archetype
    ? projectArchetype(scenarioRow, archetype).projection
    : generateProjection(scenarioRow);
  ...
  if (hardware_mix) {
    ...
    if (hb && (hb.revenue_ai_annual > 0 || hb.capex_hardware > 0)) {
      foldGpuRevenue(projection, hb, scenarioRow, {
        rfs_year: scenarioRow.phase1_complete_year || 3,
        ramp_years: 2,
        exit_year: scenarioRow.exit_year || 10,
        discount_rate_pct: scenarioRow.discount_rate_pct ?? FINANCIAL_THRESHOLDS.discount_rate_pct,
      });
    }
  }
