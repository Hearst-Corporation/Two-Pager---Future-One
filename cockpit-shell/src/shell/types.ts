...
export interface CockpitProduct {
  ...
  short: string;
  /**
   * Accent d'identité (hex). Optionnel : si absent, ThemeAccent ne pose pas
   * d'override JS et l'accent reste piloté par le fallback `--ct-accent` de
   * `tokens.css` (source unique, zéro flash CSS→JS).
   */
  color?: string;
}
