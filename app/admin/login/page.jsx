'use client';


import { Suspense, useState } from 'react';
import { useSearchParams } from 'next/navigation';
import { getBrowserClient } from '@/lib/supabase-browser';
import { safeNextPath } from '@/lib/url-helpers';
import { UI } from '@/lib/ui-strings';

export default function LoginPage() {
  return (
    <Suspense fallback={null}>
      <LoginForm />
    </Suspense>
  );
}

const AUTH_ERROR_MESSAGES = {
  otp_expired: 'This link has expired. Please request a new one.',
  access_denied: 'Access denied. Please request a new magic link.',
};

function LoginForm() {
  const sp = useSearchParams();
  // Sanitize `next` at the source: safeNextPath rejects absolute URLs,
  // protocol-relative (//evil.com) and javascript: schemes, falling back to
  // ORACLE_HOME. Covers both the password redirect and the magic-link emailRedirectTo.
  const next = safeNextPath(sp.get('next'));
  const authError = sp.get('auth_error');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [mode, setMode] = useState('magic');
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState('');
  const [err, setErr] = useState(AUTH_ERROR_MESSAGES[authError] || '');

  async function sendMagicLink(e) {
    e.preventDefault();
    setBusy(true); setErr(''); setMsg('');
    const supa = getBrowserClient();
    const { error } = await supa.auth.signInWithOtp({
      email,
      options: {
        emailRedirectTo: `${window.location.origin}/admin/auth/callback?next=${encodeURIComponent(next)}`,
      },
    });
    setBusy(false);
    if (error) setErr(error.message);
    else setMsg('Magic link sent. Check your inbox.');
  }

  async function signInWithPassword(e) {
    e.preventDefault();
    setBusy(true); setErr(''); setMsg('');
    const supa = getBrowserClient();
    const { error } = await supa.auth.signInWithPassword({ email, password });
    setBusy(false);
    if (error) {
      setErr(error.message);
      return;
    }
    window.location.href = next;
  }

  return (
    <div style={S.wrap}>
      <form onSubmit={mode === 'magic' ? sendMagicLink : signInWithPassword} style={S.card}>
        <div style={S.eyebrow}>FUTUR ONE × MISA</div>
        <h1 style={S.title}>Sign in</h1>
        <p style={S.sub}>Restricted area — invite only.</p>

        <label htmlFor="login-email" style={S.srOnly}>Email address</label>
        <input
          id="login-email"
          type="email"
          autoFocus
          required
          autoComplete="email"
          value={email}
          onChange={(e) => setEmail(e.target.value)}
          placeholder="you@company.com"
          style={S.input}
        />

        {mode === 'password' && (
          <>
            <label htmlFor="login-password" style={S.srOnly}>Password</label>
            <input
              id="login-password"
              type="password"
              required
              autoComplete="current-password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              placeholder={UI.LOGIN_PASSWORD_PLACEHOLDER}
              style={S.input}
            />
          </>
        )}

        {err && <div style={S.err}>{err}</div>}
        {msg && <div style={S.msg}>{msg}</div>}

        <button
          type="submit"
          disabled={busy}
          aria-busy={busy}
          aria-label={mode === 'magic' ? 'Send magic link' : 'Sign in'}
          style={S.btn}
        >
          {busy ? '…' : mode === 'magic' ? 'Send magic link' : 'Sign in'}
        </button>

        <button
          type="button"
          onClick={() => { setMode(mode === 'magic' ? 'password' : 'magic'); setErr(''); setMsg(''); }}
          style={S.switch}
        >
          {mode === 'magic' ? 'Use password instead' : 'Use magic link instead'}
        </button>
      </form>
    </div>
  );
}

const S = {
  wrap: {
    minHeight: '100dvh',
    background: 'var(--color-login-bg)',
    display: 'grid',
    placeItems: 'center',
    fontFamily: 'var(--font-sans)',
    padding: 24,
  },
  card: {
    width: 380,
    maxWidth: '100%',
    padding: 40,
    background: 'var(--color-login-surface)',
    border: '1px solid var(--color-login-border)',
    display: 'flex',
    flexDirection: 'column',
    gap: 16,
  },
  eyebrow: {
    fontSize: 'var(--login-text-2xs)',
    letterSpacing: 'var(--login-tracking-eyebrow)',
    fontWeight: 'var(--login-fw-heavy)',
    color: 'var(--color-login-accent)',
  },
  title: {
    fontSize: 'var(--login-text-title)',
    fontWeight: 'var(--login-fw-heavy)',
    letterSpacing: 'var(--login-tracking-title)',
    margin: 0,
    color: 'var(--color-login-text)',
  },
  sub: { fontSize: 'var(--login-text-sm)', margin: 0, color: 'var(--color-login-text-muted)' },
  input: {
    padding: 'var(--login-input-padding)',
    border: '1px solid var(--color-login-border-input)',
    background: 'var(--color-login-bg)',
    fontSize: 'var(--login-text-base)',
    fontFamily: 'inherit',
    color: 'var(--color-login-text)',
    outline: 'none',
  },
  btn: {
    padding: 'var(--login-btn-padding)',
    border: 0,
    background: 'var(--color-login-accent)',
    color: 'var(--color-login-text-inverse)',
    fontSize: 'var(--login-text-2xs)',
    fontWeight: 'var(--login-fw-heavy)',
    letterSpacing: 'var(--login-tracking-btn)',
    cursor: 'pointer',
    fontFamily: 'inherit',
  },
  switch: {
    background: 'transparent',
    border: 0,
    color: 'var(--color-login-text-muted)',
    fontSize: 'var(--login-text-2xs)',
    fontWeight: 'var(--login-fw-bold)',
    letterSpacing: 'var(--login-tracking-switch)',
    cursor: 'pointer',
    fontFamily: 'inherit',
    padding: 'var(--login-switch-padding)',
  },
  err: { fontSize: 'var(--login-text-2xs)', color: 'var(--color-login-accent)', fontWeight: 'var(--login-fw-bold)' },
  msg: { fontSize: 'var(--login-text-2xs)', color: 'var(--color-login-success)', fontWeight: 'var(--login-fw-bold)' },
  srOnly: {
    position: 'absolute',
    width: 1,
    height: 1,
    padding: 0,
    margin: -1,
    overflow: 'hidden',
    clip: 'rect(0, 0, 0, 0)',
    whiteSpace: 'nowrap',
    border: 0,
  },
};
