...
// "lost" keeps --cp-error because losing a deal is a true negative semantic outcome.
const STAGE_META = {
  prospecting: { label: 'Prospecting', color: 'var(--cp-text-muted)' },
  qualified:   { label: 'Qualified',   color: 'var(--cp-text-body)' },
  proposal:    { label: 'Proposal',    color: 'var(--cp-text-body)' },
  negotiation: { label: 'Negotiation', color: 'var(--cp-text-primary)' },
  committed:   { label: 'Committed',   color: 'var(--cp-accent)' },
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={S.kpiStrip}>
        <KpiCard label="Committed Capacity" value={committedMw} format="mw" highlight />
        <KpiCard label="Weighted Pipeline"  value={weightedMw}  format="mw" />
        <KpiCard label="Total Prospects"    value={prospects.filter(p => p.stage !== 'lost').length} format="number" />
      </div>
                ...
                <tr key={p.id} style={S.tr}>
                  ...
                  <td style={S.td}>
                    <select
                      ...
                      style={{ fontSize: 10, fontWeight: 700, padding: '4px 8px', borderRadius: 20, background: sm.bg || 'var(--cp-surface-1)', color: sm.color, border: `1px solid ${sm.color}`, cursor: 'pointer' }}
                    >
                      {STAGES.map(s => <option key={s} value={s}>{STAGE_META[s].label}</option>)}
                    </select>
                  </td>
                  <td style={{ ...S.td, fontWeight: 700, color: p.probability_pct >= 70 ? 'var(--cp-accent)' : p.probability_pct >= 40 ? 'var(--cp-text-body)' : 'var(--cp-error)' }}>
...
const S = {
  ...
  confirmYes: { fontSize: 11, fontWeight: 700, color: 'var(--cp-text-strong)', background: 'var(--cp-error-strong)', border: 'none', cursor: 'pointer', padding: '4px 6px', borderRadius: 4 },
  confirmNo: { fontSize: 10, color: 'var(--cp-text-muted)', background: 'transparent', border: '1px solid var(--cp-border)', cursor: 'pointer', padding: '4px 6px', borderRadius: 4 },
  ...
  saveBtnSm: { fontSize: 11, fontWeight: 700, color: 'var(--cp-text-strong)', background: 'var(--cp-accent)', border: 'none', cursor: 'pointer', padding: '4px 10px', borderRadius: 4, marginRight: 4 },
