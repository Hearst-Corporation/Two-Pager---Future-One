...
function QatarFlag({ size = 36 }: { size?: number }) {
  ...
  const serrationsCount = 9;
  const whiteWidth = Math.round(w * 0.28);
  const tipX = Math.round(w * 0.38);
  const stepH = h / serrationsCount;

  let points = `0,0 ${whiteWidth},0`;
  for (let i = 0; i < serrationsCount; i++) {
    const midY = Math.round(stepH * i + stepH / 2);
    const endY = Math.round(stepH * (i + 1));
    points += ` ${tipX},${midY} ${whiteWidth},${endY}`;
  }
  points += ` ${whiteWidth},${h} 0,${h}`;
